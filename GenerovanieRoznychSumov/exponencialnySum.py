...
# Parametre pre Exponenciálne rozdelenie
lam = 0.25   # lambda (intenzita) 1/λ=4,
size = (256, 256)

# Generovanie Exponenciálneho šumu
exponential_noise = np.random.exponential(scale=1/lam, size=size).astype(np.float32)

# Normalizácia na rozsah [0, 30]
exponential_noise = exponential_noise - exponential_noise.min()
exponential_noise = (exponential_noise / exponential_noise.max()) * 30
exponential_noise = exponential_noise.astype(np.int16)
...
noisy_image = base_image + exponential_noise
...
plt.subplot(1, 2, 1)
plt.title("Obrázok s exponenciálnym šumom")
